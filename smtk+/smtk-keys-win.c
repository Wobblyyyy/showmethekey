					   ...
					   gpointer user_data)
{
	...
	pango_font_description_set_size(font, allocation->height *
						      (win->fontsize / 100.0) *
						      PANGO_SCALE);
...
static void smtk_keys_win_class_init(SmtkKeysWinClass *win_class)
{
	...
	obj_properties[PROP_FONTSIZE] = g_param_spec_int(
		"fontsize", "Font Size Coefficient", "Font Size Coefficient", 1,
		1000, 10, G_PARAM_CONSTRUCT_ONLY | G_PARAM_READWRITE);
			     ...
			     gint fontsize, GError **error)
{
	SmtkKeysWin *win = g_object_new(
		...
		_("Floating Window - SMTK+"), "icon-name", "showmethekey",
